...
files_modified:
  ...
  - components/admin/ProjectFeaturedToggle.tsx
autonomous: false
...
must_haves:
  ...
  artifacts:
    ...
    - path: "components/admin/ProjectStatusToggle.tsx"
      ...
      min_lines: 40
    - path: "components/admin/ProjectFeaturedToggle.tsx"
      provides: "Featured toggle with optimistic UI"
      min_lines: 40
  key_links:
    ...
    - from: "components/admin/ProjectFeaturedToggle.tsx"
      to: "convex/projects.ts"
      via: "useMutation to updateFeatured"
...
<task type="auto">
  ...
  <action>
    Install required dependencies for form handling and validation:

    npm install react-hook-form @hookform/resolvers zod react-hot-toast

    Why each package:
    - react-hook-form: Type-safe form library with React integration
    - @hookform/resolvers: Connects Zod validation to react-hook-form
    - zod: Schema validation (matching Convex schema patterns)
    - react-hot-toast: User feedback for mutations (success/error messages)

    These are required before creating form components in subsequent tasks.
  </action>
  <verify>
    1. Check package.json includes all four packages
    2. Run: npm list react-hook-form @hookform/resolvers zod react-hot-toast
  </verify>
  <done>
    Form dependencies installed and available for import in task components.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    4. Import ProjectStatusToggle, ProjectFeaturedToggle, DeleteProjectButton components (created in later tasks)
    ...
    5. Create ProjectsListPage component:
       ...
       - Render:
         ...
         * Projects table or grid:
           ...
           - Actions: Edit button (links to /admin/projects/[id]/edit), ProjectStatusToggle, ProjectFeaturedToggle, DeleteProjectButton
  ...
  <verify>
    ...
    5. Check ProjectStatusToggle and ProjectFeaturedToggle components imported
...
<task type="auto">
  <name>Create project status toggle, featured toggle, and delete components</name>
  <files>components/admin/ProjectStatusToggle.tsx, components/admin/ProjectFeaturedToggle.tsx, components/admin/DeleteProjectButton.tsx</files>
  <action>
    ...
    2. Create components/admin/ProjectFeaturedToggle.tsx:
       - Mark 'use client'
       - Import useMutation from convex/react, api, useState, toast
       - Accept props: projectId, initialFeatured (boolean)
       - Use useState for optimisticFeatured (initialized with initialFeatured)
       - Use useMutation for api.projects.update
       - On toggle:
         * Calculate new featured value (!currentValue)
         * Update optimisticFeatured immediately
         * Call mutation with { id: projectId, featured: newValue }
         * Show toast on success
         * Rollback optimisticFeatured on error
       - Render button with star icon (filled if featured, outline if not)

    3. Create components/admin/DeleteProjectButton.tsx:
       - Mark 'use client'
    ...
    Why ProjectFeaturedToggle is separate from ProjectStatusToggle:
    - Different mutation targets (updateStatus vs update with featured field)
    - Different UI (star icon vs status badge)
    - Independent optimistic states
  </action>
  <verify>
    1. Check components/admin/ProjectStatusToggle.tsx exists with optimistic updates
    2. Check components/admin/ProjectFeaturedToggle.tsx exists with optimistic updates and star icon
    3. Check components/admin/DeleteProjectButton.tsx exists with confirmation modal
    4. Check all components use useMutation hooks
    5. Run: npm run typecheck
  </verify>
  <done>
    Status toggle, featured toggle (both with optimistic UI), and delete button with confirmation dialog.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    Complete admin interface for managing projects: list view, create form, edit form, status toggles, featured toggles, delete with confirmation.
  </what-built>
  <how-to-verify>
    ...
    5. Test FEATURED TOGGLE:
       - Click featured toggle (star icon) on test project
       - Verify star changes from outline to filled (or vice versa)
       - Verify change persists on page refresh
...
5. Featured toggle works with optimistic UI (star icon)
6. Delete requires confirmation and removes project
7. All forms show validation errors
8. All mutations succeed with authenticated session
</verification>
...
- Featured toggle with optimistic UI (star icon)
